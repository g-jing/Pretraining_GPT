...
def train(args, train_dataset, model, tokenizer):
    ...
    train_dataloader = DataLoader(train_dataset, collate_fn=train_dataset.collate, sampler=train_sampler, batch_size=args.train_batch_size)
...
def main():
    ...
    for one in members:
        if getattr(configuration, one)!=None:
            setattr(args, one, getattr(configuration, one))
    ...
    if args.block_size <= 0:
        args.block_size = 1024 # tokenizer.max_len_single_sentence  # Our input block size will be the max possible for the model
    args.block_size = min(args.block_size, 1024) # tokenizer.max_len_single_sentence
